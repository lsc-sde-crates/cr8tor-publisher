...
class TestDLTDataRetriever:
    ...
    @patch("app.core.databricks.handle_restapi_request")
    def test_get_table_metadata_success(
        ...
        # mock the response from databricks.handle_restapi_request() function
        mock_response = {
            "columns": [
        ...
        self.retriever.access_token = MagicMock()
    ...
    @patch("app.core.databricks.get_access_token")
    ...
    def test_initialize_dlt_pipeline_unsupported_destination(
        ...
        self.retriever.destination.format = "UnsupportedType"

        with pytest.raises(
            UnknownDestinationModule,
            match="Destination filestore is not one of the standard dlt destinations",
        ):
            self.retriever._initialize_dlt_pipeline()  # noqa: SLF001
